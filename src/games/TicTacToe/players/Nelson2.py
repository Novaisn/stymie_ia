...
class Nelson2(TicTacToePlayer):
    ...
    def minimax(self, state: TicTacToeState, depth: int, alpha: int = -sys.maxsize, beta: int = sys.maxsize,
                is_initial_node: bool = True):
        # first we check if we are in a terminal node (victory, draw or loose)
        if state.is_finished():
            return {
                TicTacToeResult.WIN: 4,
                TicTacToeResult.LOOSE: -4,
    ...
    def get_action(self, state: TicTacToeState):
        return TicTacToeAction(self.minimax(state, 3))
